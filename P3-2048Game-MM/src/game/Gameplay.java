...
public class Gameplay extends JPanel implements KeyListener {
	...
	//This is used to know when the game is finished because the user won
	...
	public static void startGame()
	{
		...
		//With this line we add an object (in this case our game to the JPanel contained by the JFrame)
		frame.add(game);
	...
	{
		//Here we call the 'paint' method of JPanel
		super.paint(graphic);
		...
		graphic.drawString( "Use las flechas de su teclado para comenzar a jugar", 180, 40 );

		//  https://docs.oracle.com/javase/7/docs/api/java/awt/Graphics.html#fillRect(int,%20int,%20int,%20int)
		graphic.setColor( Color.white );

		graphic.fillRect( 150, 60, 270, 270 );
		for ( int i = 0; i < 4; i++ )
		{
			for ( int j = 0; j < 4; j++ )
			{
				renderNode( graphic, gameBoard.board[i][j], j * 70 + 150, i * 70 + 70 );
			}
		}
		if (gameBoard.nextMoveCapacity() == false)
		{
			graphic.drawString("GAME OVER", 260, 260);
			frame.removeKeyListener(game);
		}
		if(gameBoard.getHighestNode() >= 2048) {
			graphic.drawString("YOU WIN", 260, 260);
			frame.removeKeyListener(game);
		}
